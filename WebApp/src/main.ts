  ...
  capture(utils: any) {
    try {
      ...
      for (let i = 0; i < this.eyesDetected.size(); ++i) {
        ...
        this.cv.rectangle(this.destinationMatrix, point1, point2, [
          255,
          0,
          0,
          255,
      ...
      context.strokeText(
        this.hasMask ? "Has face mask" : "no face mask",
        20,
        20
      );
...
module.exports = function (cv: any, utils: any) {
  const app = new App(cv, document.querySelector("#source"), "output");
  app.capture(utils);
};
